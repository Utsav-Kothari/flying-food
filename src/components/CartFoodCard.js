...
const CartFoodCard=({imageId,name,price,description,itemAttribute})=>{
    console.log({name});
    return(
        <div className="flex flex-col h-inherit w-28 m-4">
        <img src={IMG_CDN_URL + imageId} className='h-28 rounded-md'/>
        <div className="font-bold text-xl m-0.5 pt-0.5">{name.slice(0,20)}...</div>
